...
class AutoMod(commands.Cog):
    ...
    def _save_config(self):
        save_json_file_atomic(CONFIG_FILE, self.config, indent=4, ensure_ascii=False)
    ...
    async def automod_status(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="🛡️ Configuración AutoMod", color=Theme.get_color(interaction.guild.id, "primary")
        )
        ...
        embed.add_field(
            ...
            value=f"{len(conf.get('bad_words', []))} palabras",
            inline=False,
        )
        embed.add_field(
            name="📊 Límite Mayúsculas",
            value=f"{int(conf.get('caps_threshold', 0.7)*100)}%",
            inline=True,
        )
    ...
    async def automod_toggle(self, interaction: discord.Interaction, filtro: str, estado: bool):
        ...
        await interaction.response.send_message(
            f"✅ Filtro **{filtro}** establecido a: {'Activado' if estado else 'Desactivado'}"
        )
    ...
    async def automod_addword(self, interaction: discord.Interaction, palabra: str):
        ...
        if palabra in words:
            await interaction.response.send_message(
                "⚠️ Esa palabra ya está en la lista.", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"✅ Palabra añadida: ||{palabra}||", ephemeral=True
        )
    ...
    async def automod_delword(self, interaction: discord.Interaction, palabra: str):
        ...
        if palabra not in words:
            await interaction.response.send_message(
                "⚠️ Esa palabra no está en la lista.", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"✅ Palabra eliminada: ||{palabra}||", ephemeral=True
        )

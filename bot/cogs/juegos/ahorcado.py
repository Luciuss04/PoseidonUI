...
class GuessModal(discord.ui.Modal, title="Adivina una letra"):
    letter = discord.ui.TextInput(
        label="Letra", min_length=1, max_length=1, placeholder="Escribe una sola letra..."
    )
...
class SolveModal(discord.ui.Modal, title="Adivina la palabra"):
    word = discord.ui.TextInput(
        label="Palabra completa", min_length=2, placeholder="Escribe la palabra completa..."
    )
...
class AhorcadoView(discord.ui.View):
    ...
    def get_embed(self, guild_id):
        color = (
            Theme.get_color(guild_id, "success")
            if self.solved
            else (
                Theme.get_color(guild_id, "error")
                if self.mistakes >= self.max_mistakes
                else Theme.get_color(guild_id, "primary")
            )
        )
        ...
        desc += (
            f"**Letras usadas:** {', '.join(sorted(self.guesses)) if self.guesses else 'Ninguna'}"
        )
    ...
    async def process_guess(self, interaction: discord.Interaction, char):
        if char in self.guesses:
            await interaction.response.send_message(
                f"Ya usaste la letra **{char}**.", ephemeral=True
            )
            return
        ...
        await interaction.response.edit_message(
            embed=self.get_embed(interaction.guild.id), view=self
        )
        ...
        if text == self.word:
            ...
            if self.mistakes >= self.max_mistakes:
                ...
                await interaction.followup.send(
                    f"**{text}** no es la palabra correcta. Pierdes un intento.", ephemeral=True
                )
...
class Ahorcado(commands.Cog):
    ...
    async def ahorcado(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=view.get_embed(interaction.guild.id), view=view
        )

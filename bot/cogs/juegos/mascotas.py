...
class BattleView(discord.ui.View):
    ...
    async def update_embed(self, interaction, final=False):
        color = Theme.get_color(
            interaction.guild.id, "error" if final and self.p_hp <= 0 else "success"
        )
        ...
        # Log formateado
        log_text = ""
        for log_line in self.logs[-4:]:
            if "atacó" in log_line:
                log_text += f"💢 {log_line}\n"
            elif "curó" in log_line:
                log_text += f"💚 {log_line}\n"
            elif "CRÍTICO" in log_line:
                log_text += f"🔥 **{log_line}**\n"
            else:
                log_text += f"{log_line}\n"
        ...
        e_stats = (
            f"❤️ HP: {make_bar(self.e_hp, self.e_max_hp, '🟥')}\n🤖 IA Nvl: {self.enemy['level']}"
        )
        ...
        embed.add_field(
            name="⚡ VS ⚡", value=f"**{self.enemy['name']}** {e_elem_icon}", inline=False
        )
        embed.add_field(name="Estadísticas del Oponente", value=e_stats, inline=False)
        ...
        if final:
            ...
            embed.set_footer(
                text=f"Turno de: {self.user.display_name} • Usa tus habilidades sabiamente"
            )
    ...
    async def enemy_turn(self, interaction):
        ...
        if action == "heal":
            ...
            mult = self._get_element_multiplier(
                self.e_info.get("element", "normal"), self.p_info.get("element", "normal")
            )
    ...
    async def attack(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        mult = self._get_element_multiplier(
            self.p_info.get("element", "normal"), self.e_info.get("element", "normal")
        )
    ...
    async def special(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.p_energy < 40:
            await interaction.response.send_message(
                "⚠️ No tienes suficiente energía (40).", ephemeral=True
            )
            return
    ...
    async def heal(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.p_energy < 30:
            await interaction.response.send_message(
                "⚠️ No tienes suficiente energía (30).", ephemeral=True
            )
            return
...
class Mascotas(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.pet_types = {
            ...
            "gato": {
                "emoji": "🐱",
                "desc": "Independiente pero cariñoso.",
                "food": "Pescado",
                "attack": 8,
                "defense": 8,
                "element": "normal",
            ...
            "lobo_mistico": {
                "emoji": "🐺",
                "desc": "Evolución de Perro. Un guardián espiritual.",
                "food": "Carne Sagrada",
                "attack": 25,
                "defense": 15,
                "element": "tierra",
            ...
            "leon_solar": {
                "emoji": "🦁",
                "desc": "Evolución de Gato. Brilla con luz propia.",
                "food": "Ambrosía",
                "attack": 20,
                "defense": 20,
                "element": "luz",
            ...
            "bahamut": {
                "emoji": "🐉",
                "desc": "Evolución de Dragón. El rey de los dragones.",
                "food": "Cristales de Maná",
                "attack": 35,
                "defense": 25,
                "element": "fuego",
                "emoji_elem": "🔥",
            ...
            "mecha_prime": {
                "emoji": "🦾",
                "desc": "Evolución de Robot. Tecnología suprema.",
                "food": "Núcleo de Fusión",
                "attack": 28,
                "defense": 28,
                "element": "electrico",
                "emoji_elem": "⚡",
            ...
            "bestia_divina": {
                "emoji": "🐗",
                "desc": "Evolución de Hamster. Imparable.",
                "food": "Néctar",
                "attack": 15,
                "defense": 45,
                "element": "tierra",
            ...
            "invasor_cosmico": {
                "emoji": "🛸",
                "desc": "Evolución de Alien. Conquistador de mundos.",
                "food": "Materia Oscura",
                "attack": 30,
                "defense": 20,
                "element": "oscuridad",
            ...
            "espectro_real": {
                "emoji": "💀",
                "desc": "Evolución de Fantasma. Terror nocturno.",
                "food": "Almas",
                "attack": 25,
                "defense": 30,
                "element": "oscuridad",
            ...
            "t_rex_titan": {
                "emoji": "🦕",
                "desc": "Evolución de Dinosaurio. El depredador definitivo.",
                "food": "Meteorito",
                "attack": 40,
                "defense": 10,
                "element": "fuego",
            ...
            "alicornio_sagrado": {
                "emoji": "🌈",
                "desc": "Evolución de Unicornio. Luz de esperanza.",
                "food": "Luz Pura",
                "attack": 20,
                "defense": 35,
                "element": "luz",
            ...
            "rey_slime": {
                "emoji": "👑",
                "desc": "Evolución de Slime. El más grande de todos.",
                "food": "Océano",
                "attack": 15,
                "defense": 50,
                "element": "agua",
    ...
    def _check_vital_signs(self, pet):
        ...
        if pet["happiness"] <= 0:
            return (
                False,
                f"🏃 **{pet['name']}** se ha escapado de casa porque estaba muy triste... 💔",
            )
    ...
    @app_commands.describe(
        ...
        nombre="Nombre (adoptar/renombrar) / Apuesta (duelo)",
        tipo="Tipo (adoptar)",
    )
    ...
    async def mascota(
        self, interaction: discord.Interaction, accion: str, nombre: str = None, tipo: str = None
    ):
        ...
        if accion == "adoptar":
            if pet:
                await interaction.response.send_message(
                    "❌ Ya tienes una mascota. Usa `/mascota liberar` si quieres otra.",
                    ephemeral=True,
                )
                return
            ...
            if not tipo or not nombre:
                await interaction.response.send_message(
                    "⚠️ Debes especificar un `tipo` y un `nombre` para adoptar.", ephemeral=True
                )
                return
            ...
            if monedas:
                if not monedas.remove_balance(interaction.user.id, costo):
                    await interaction.response.send_message(
                        f"❌ Adoptar una mascota cuesta 💰 {costo}.", ephemeral=True
                    )
                    return
            ...
            await interaction.response.send_message(
                f"🎉 ¡Felicidades! Has adoptado a **{nombre}** el {info['emoji']} ({tipo}) por 💰 {costo}."
            )

        elif accion == "ver":
            if not pet:
                await interaction.response.send_message(
                    "😢 No tienes mascota. Usa `/mascota adoptar`.", ephemeral=True
                )
                return
            ...
            embed = discord.Embed(
                title=f"Estado de {pet['name']} {info['emoji']} ({pet['type'].capitalize()})",
            ...
            embed.add_field(
                name="Récord",
                value=f"🏆 {pet.get('wins', 0)} - 💀 {pet.get('losses', 0)}",
                inline=True,
            )
            ...
            if pet["hunger"] >= 100:
                await interaction.response.send_message(
                    f"🤢 **{pet['name']}** está llenísimo. No quiere comer más.", ephemeral=True
                )
                return
            ...
            if monedas:
                if not monedas.remove_balance(interaction.user.id, costo_comida):
                    await interaction.response.send_message(
                        f"❌ No tienes 💰 {costo_comida} para comprar comida.", ephemeral=True
                    )
                    return
            ...
            info = self.pet_types.get(pet["type"], {"emoji": "❓"})
            await interaction.response.send_message(
                f"😋 Le diste de comer a **{pet['name']}** (Coste: 💰 {costo_comida}). ¡Se ve feliz! (+5 XP){msg_extra}"
            )
            ...
            if pet["hunger"] < 20:
                await interaction.response.send_message(
                    f"😟 **{pet['name']}** tiene mucha hambre para jugar. Aliméntalo primero.",
                    ephemeral=True,
                )
                return
            ...
            if pet["energy"] < 10:
                await interaction.response.send_message(
                    f"😴 **{pet['name']}** está muy cansado. Déjalo dormir un poco.", ephemeral=True
                )
                return
            ...
            self._save_pet(user_id, pet)
            await interaction.response.send_message(
                f"🎾 Jugaste con **{pet['name']}**. ¡Se divirtió mucho! (+10 XP){msg_extra}"
            )
            ...
            if pet["energy"] < 30:
                await interaction.response.send_message(
                    f"😴 **{pet['name']}** está demasiado cansado para entrenar.", ephemeral=True
                )
                return

            if pet["hunger"] < 30:
                await interaction.response.send_message(
                    f"😟 **{pet['name']}** tiene hambre. No puede entrenar bien.", ephemeral=True
                )
                return
            ...
            self._save_pet(user_id, pet)
            await interaction.response.send_message(
                f"🏋️ **{pet['name']}** ha entrenado duro. ¡Se vuelve más fuerte! (+25 XP){msg_extra}"
            )
            ...
            if pet["energy"] >= 90:
                await interaction.response.send_message(
                    f"👀 **{pet['name']}** no tiene sueño.", ephemeral=True
                )
                return
            ...
            self._save_pet(user_id, pet)
            await interaction.response.send_message(
                f"💤 **{pet['name']}** ha dormido una siesta reparadora. ¡Energía al máximo!"
            )
            ...
            if (
                "last_exploration" in pet and time.time() - pet["last_exploration"] < 300
            ):  # 5 min cooldown
                remaining = int(300 - (time.time() - pet["last_exploration"]))
                await interaction.response.send_message(
                    f"⏳ **{pet['name']}** está descansando de su última aventura. Espera {remaining} segundos.",
                    ephemeral=True,
                )
                return
            ...
            if pet["energy"] < 20:
                await interaction.response.send_message(
                    f"😴 **{pet['name']}** está muy cansado para salir.", ephemeral=True
                )
                return
            ...
            if outcome <= 25:  # 25% dinero común
                ...
                msg = (
                    f"🍎 **{pet['name']}** encontró frutas silvestres y se las comió. (+40 Hambre)"
                )
            ...
            if nombre and nombre.isdigit():
                ...
                await interaction.response.send_message(
                    "⚠️ Debes ingresar la cantidad a apostar en el campo 'nombre'.", ephemeral=True
                )
                return

            if bet < 0:
                await interaction.response.send_message(
                    "⚠️ La apuesta debe ser positiva.", ephemeral=True
                )
                return
            ...
            if monedas:
                if not monedas.remove_balance(interaction.user.id, bet):
                    await interaction.response.send_message(
                        "❌ No tienes suficiente dinero para esa apuesta.", ephemeral=True
                    )
                    return
            ...
            if not nombre:
                await interaction.response.send_message(
                    "⚠️ Debes especificar el nombre del item (comida, superalimento, medicina, juguete).",
                    ephemeral=True,
                )
                return
            ...
            if item == "comida":
                ...
                if monedas and not monedas.remove_balance(interaction.user.id, cost):
                    await interaction.response.send_message(
                        f"❌ No tienes 💰 {cost}.", ephemeral=True
                    )
                    return
                pet["hunger"] = min(100, pet["hunger"] + 30)
                msg_extra = self._add_xp(pet, 5)
                ...
                await interaction.response.send_message(
                    "❌ Item no encontrado. Mira la `/mascota tienda`.", ephemeral=True
                )
                return

            self._save_pet(user_id, pet)
            await interaction.response.send_message(
                f"🛍️ Compraste **{item}** por 💰 {cost}.\n**{pet['name']}** {effect}"
            )

        elif accion == "liberar":
            if not pet:
                await interaction.response.send_message(
                    "No tienes mascota para liberar.", ephemeral=True
                )
                return
            ...
            self._save_pets()
            await interaction.response.send_message(
                "👋 Has liberado a tu mascota. Esperamos que encuentre un buen hogar.",
                ephemeral=True,
            )
            ...
            if current_type not in self.evolution_map:
                await interaction.response.send_message(
                    f"❌ Tu **{current_type}** no puede evolucionar más (o no tiene evolución conocida).",
                    ephemeral=True,
                )
                return
            ...
            if pet["level"] < 10:
                await interaction.response.send_message(
                    f"❌ **{pet['name']}** necesita ser Nivel 10 para evolucionar (Actual: {pet['level']}).",
                    ephemeral=True,
                )
                return
            ...
            if monedas and not monedas.remove_balance(interaction.user.id, cost):
                await interaction.response.send_message(
                    f"❌ La evolución cuesta 💰 {cost}.", ephemeral=True
                )
                return
            ...
            if not self.pets:
                await interaction.response.send_message(
                    "❌ No hay mascotas registradas aún.", ephemeral=True
                )
                return

            # Ordenar por nivel (desc) y luego XP (desc)
            sorted_pets = sorted(
                self.pets.items(), key=lambda item: (item[1]["level"], item[1]["xp"]), reverse=True
            )
            ...
            embed = discord.Embed(
                title="🏆 Ranking Global de Mascotas",
                description=desc,
                color=Theme.get_color(interaction.guild.id, "primary"),
            )
            embed.set_footer(text=Theme.get_footer_text(interaction.guild.id))
            ...
            if not pet:
                await interaction.response.send_message(
                    "❌ No tienes mascota para renombrar.", ephemeral=True
                )
                return

            if not nombre:
                await interaction.response.send_message(
                    "⚠️ Debes especificar el nuevo nombre en el campo `nombre`.", ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                f"✏️ Has renombrado a tu mascota.\nDe **{old_name}** a **{nombre}**."
            )

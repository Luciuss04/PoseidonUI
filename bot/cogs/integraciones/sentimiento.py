...
class SentimentAI(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.KEYWORDS_POSITIVE = [
            "gracias", "genial", "increíble", "buen", "mejor", "feliz", "contento", "ayuda", "solucionado",
            "perfecto", "excelente", "amo", "adoro", "bien", "fantástico", "maravilla", "recomiendo"
        ]
        
        self.KEYWORDS_NEGATIVE = [
            "mal", "peor", "asco", "horrible", "error", "fallo", "malo", "odio", "enfadado", "triste",
            "pésimo", "basura", "inútil", "no funciona", "decepción", "estafa", "robo", "lento"
        ]
        
        self.KEYWORDS_TOXIC = [
            "idiota", "estúpido", "imbécil", "tonto", "mierda", "joder", "puto", "puta", "cabrón",
            "muérete", "basura", "rata", "manco", "lárgate", "inservible", "estupidez"
        ]
    ...
    def analyze_sentiment(self, text: str):
        ...
        if toxicity > 0.5: label = "Tóxico"
        
        return {
            "score": score,
            "label": label,
            "toxicity": toxicity,
            "emoji": "😊" if label == "Positivo" else ("😠" if label == "Tóxico" else ("😟" if label == "Negativo" else "😐"))
        }
    ...
    async def on_message(self, message):
        ...
        if analysis["toxicity"] >= 0.7:
            ...
            try:
                from bot.config import get_guild_setting, LOG_CHANNEL_ID
                ...
                if log_ch:
                    ...
                    embed.add_field(name="Nivel de Toxicidad", value=f"{int(analysis['toxicity'] * 100)}%")

...
class GamingProfiles(commands.Cog):
    ...
    @app_commands.command(
        name="gaming_link", description="Vincula tu cuenta de juego (LoL/Valorant)"
    )
    @app_commands.describe(
        juego="Elige el juego",
        nombre="Tu nombre de invocador/usuario",
        tag="Tu Tag (Ej: EUW, 1234)",
    )
    ...
    async def gaming_link(
        self, interaction: discord.Interaction, juego: str, nombre: str, tag: str
    ):
    ...
    @app_commands.command(
        name="perfil", description="Muestra tu perfil gaming o el de otro usuario"
    )
    async def perfil(self, interaction: discord.Interaction, usuario: discord.Member = None):
        ...
        embed.set_footer(
            text=f"{Theme.get_footer_text(interaction.guild_id)} • Poseidon Gaming Engine"
        )

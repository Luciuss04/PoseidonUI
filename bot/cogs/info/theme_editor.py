...
class HexInputModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("❌ Error al procesar el color.", ephemeral=True)
            return
...
class ThemeEditorView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        row=0,
        options=[
            discord.SelectOption(label="🌊 Océano Profundo", value="ocean", description="Azul / Cian"),
            discord.SelectOption(label="🍃 Bosque Encantado", value="forest", description="Verde / Naturaleza"),
            discord.SelectOption(label="🤖 Cyberpunk 2077", value="cyber", description="Amarillo / Neón"),
            discord.SelectOption(label="✨ Sueño Pastel", value="pastel", description="Colores suaves"),
            discord.SelectOption(label="🌑 Modo Oscuro", value="dark", description="Gris / Minimalista"),
            discord.SelectOption(label="🧛 Dracula", value="dracula", description="Púrpura / Oscuro"),
        ]
    )
    ...
    @discord.ui.select(
        ...
        row=1,
        options=[
            discord.SelectOption(label="Primary", value="primary", description="Color principal de los embeds", emoji="🟦"),
            discord.SelectOption(label="Secondary", value="secondary", description="Detalles secundarios", emoji="🟪"),
            discord.SelectOption(label="Success", value="success", description="Operaciones exitosas", emoji="🟩"),
            discord.SelectOption(label="Error", value="error", description="Errores y fallos", emoji="🟥"),
            discord.SelectOption(label="Warning", value="warning", description="Advertencias", emoji="🟧"),
            discord.SelectOption(label="Info", value="info", description="Información general", emoji="ℹ️"),
        ]
    )
    ...
    async def preview_theme(self, interaction: discord.Interaction, button: discord.ui.Button):
        c = self.draft_colors
        e_success = discord.Embed(title="✅ Éxito", description="Operación completada.", color=c.get("success", 0x00FF00))
        e_error = discord.Embed(title="❌ Error", description="Algo salió mal.", color=c.get("error", 0xFF0000))
        e_success.set_footer(text=self.draft_footer)
        
        await interaction.response.send_message(
            content="**Vista Previa Rápida:**",
...
class ThemeEditor(commands.Cog):
    ...
    @app_commands.command(name="crear_tema", description="Abre el estudio de diseño para crear un tema personalizado")
    async def crear_tema(self, interaction: discord.Interaction):
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("⛔ Solo administradores pueden crear temas.", ephemeral=True)
            return
    ...
    async def borrar_tema(self, interaction: discord.Interaction, nombre: str):
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("⛔ Solo administradores pueden eliminar temas.", ephemeral=True)
            return

        # Verificar si es un tema custom
        if not nombre.startswith("custom_"):
             await interaction.response.send_message("❌ Solo se pueden borrar temas personalizados.", ephemeral=True)
             return

        # Intentar borrar
        if Theme.delete_custom_theme(nombre):
            await interaction.response.send_message(f"🗑️ Tema `{nombre}` eliminado correctamente.", ephemeral=True)
        else:
            await interaction.response.send_message(f"❌ No se encontró el tema `{nombre}`.", ephemeral=True)

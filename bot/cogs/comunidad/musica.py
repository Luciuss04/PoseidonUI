...
try:
    ...
    print(
        "⚠️ [Musica] La librería 'spotipy' no está instalada. El soporte para Spotify será limitado (vía yt-dlp)."
    )
...
class Musica(commands.Cog):
    ...
    async def musica_debug(self, interaction: discord.Interaction):
        ...
        # 1. Check Opus
        opus_status = (
            "✅ Cargado" if discord.opus.is_loaded() else "❌ No cargado (Voice no funcionará)"
        )
        ...
        ffmpeg_status = (
            f"✅ Encontrado: {ffmpeg_path}" if ffmpeg_path else "❌ NO ENCONTRADO en PATH"
        )
        ...
        embed = discord.Embed(
            title="🕵️ Diagnóstico de Música", color=Theme.get_color(interaction.guild.id, "warning")
        )
    ...
    async def _play_next(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.set_author(
                name="Poseidon Music Player", icon_url=self.bot.user.display_avatar.url
            )
    ...
    async def play(self, interaction: discord.Interaction, busqueda: str):
        ...
        if not voice_client:
            ...
            try:
                ...
                voice_client = await asyncio.wait_for(
                    channel.connect(timeout=20, reconnect=True, self_deaf=True), timeout=25
                )
        ...
        try:
            ...
            if "open.spotify.com" in busqueda:
                ...
                if not client_id or not client_secret:
                    ...
                    await interaction.followup.send(
                        "⚠️ Sin claves de API (Usando modo lento/fallback)..."
                    )
                    try:
                        ...
                        if "entries" in info:
                            ...
                            await interaction.channel.send(
                                f"✅ Spotify Fallback: **{len(tracks_to_add)}** canciones encontradas."
                            )
                        else:
                            # Single track
                        ...
                        await interaction.followup.send(
                            f"❌ Error en modo sin claves: {e}. El dueño debe configurar SPOTIFY_CLIENT_ID."
                        )
                        return
                    ...
                    if "track" in busqueda:
                        ...
                        for item in tracks:
                            ...
                            if track:
                                ...
                                count += 1
                        await interaction.channel.send(
                            f"✅ Playlist detectada: **{count}** canciones añadidas a la cola."
                        )
                        ...
                        for track in tracks:
                            ...
                            count += 1
                        await interaction.channel.send(
                            f"✅ Álbum detectado: **{count}** canciones añadidas a la cola."
                        )
            ...
            if not entries:
                await interaction.followup.send(
                    f"❌ No encontré nada para: {busqueda}", ephemeral=True
                )
                return
            ...
            if added_count > 1:
                await interaction.channel.send(
                    f"✅ Playlist de YouTube: **{added_count}** canciones añadidas."
                )
            elif added_count == 1:

        ...
        if accion == "proponer":
            if not objetivo:
                await interaction.response.send_message(
                    "⚠️ Debes mencionar a quien quieres proponerle matrimonio.", ephemeral=True
                )
                return
            ...
            if target_id == user_id:
                await interaction.response.send_message(
                    "❌ No puedes casarte contigo mismo.", ephemeral=True
                )
                return

            if objetivo.bot:
                await interaction.response.send_message(
                    "❌ No puedes casarte con un bot.", ephemeral=True
                )
                return
            ...
            if self._get_partner(target_id):
                await interaction.response.send_message(
                    f"❌ **{objetivo.display_name}** ya está casado.", ephemeral=True
                )
                return

            self.proposals[target_id] = user_id
            await interaction.response.send_message(
                f"💍 **{interaction.user.mention}** le ha propuesto matrimonio a **{objetivo.mention}**.\nUsa `/love aceptar` para decir que sí."
            )

        elif accion == "aceptar":
            if user_id not in self.proposals:
                await interaction.response.send_message(
                    "😢 Nadie te ha propuesto matrimonio recientemente.", ephemeral=True
                )
                return
            ...
            if self._get_partner(proposer_id):
                await interaction.response.send_message(
                    "❌ El proponente ya se casó con alguien más.", ephemeral=True
                )
            ...
            embed = discord.Embed(
                ...
                description=f"**{interaction.user.mention}** y **<@{proposer_id}>** han unido sus vidas en sagrado matrimonio.",
            ...
            embed.set_footer(
                text=f"{Theme.get_footer_text(interaction.guild.id)} • ¡Que el amor perdure para siempre!"
            )
            ...
            self._save_data()
            await interaction.response.send_message(
                f"💔 **{interaction.user.mention}** se ha divorciado. El dinero conjunto se ha repartido."
            )
            ...
            if target_id not in self.data:
                await interaction.response.send_message(
                    f"ℹ️ **{target.display_name}** está soltero/a.", ephemeral=True
                )
                return
            ...
            embed = discord.Embed(
                title=f"💖 Estado Civil de {target.display_name}",
            ...
            embed.add_field(
                name="💗 Puntos de Amor", value=str(data.get("love_points", 0)), inline=True
            )
            ...
            if not texto:
                await interaction.response.send_message(
                    "⚠️ Especifica el nombre del anillo en 'texto'.", ephemeral=True
                )
                return
            ...
            if monedas:
                if not monedas.remove_balance(interaction.user.id, cost):
                    await interaction.response.send_message(
                        f"❌ Un nuevo anillo cuesta 💰 {cost}.", ephemeral=True
                    )
                    return
            ...
            self._save_data()
            await interaction.response.send_message(
                f"💎 Has comprado un **{texto}** por 💰 {cost}."
            )
            ...
            if not texto:
                await interaction.response.send_message(
                    "⚠️ Especifica el nombre de tu hijo/a en 'texto'.", ephemeral=True
                )
                return
            ...
            if monedas:
                if not monedas.remove_balance(interaction.user.id, cost):
                    await interaction.response.send_message(
                        f"❌ La adopción cuesta 💰 {cost}.", ephemeral=True
                    )
                    return
            ...
            self._save_data()
            await interaction.response.send_message(
                f"👶 ¡Felicidades! **{texto}** ahora es parte de la familia (Coste: 💰 {cost})."
            )

        elif accion == "interactuar":
            if not self._get_partner(user_id):
                await interaction.response.send_message(
                    "❌ Necesitas pareja para esto.", ephemeral=True
                )
                return
            ...
            if tipo not in validos:
                await interaction.response.send_message(
                    "⚠️ Tipos válidos en 'texto': besar, abrazar, cita", ephemeral=True
                )
                return
            ...
            if now - last < 300:
                await interaction.response.send_message(
                    "⏳ ¡Hey, tranquilos! Esperen unos minutos.", ephemeral=True
                )
                return
            ...
            if tipo == "besar":
                ...
                if monedas:
                    if not monedas.remove_balance(interaction.user.id, cost):
                        await interaction.response.send_message(
                            f"❌ Una cita cuesta 💰 {cost}.", ephemeral=True
                        )
                        return
                points = 50
            ...
            try:
                ...
                await interaction.response.send_message(
                    "⚠️ Ingresa la cantidad en 'texto'.", ephemeral=True
                )
                return

            if amount == 0:
                await interaction.response.send_message(
                    f"💰 Saldo conjunto: {self.data[user_id].get('bank', 0)}", ephemeral=True
                )
                return
            ...
            if amount > 0:
                if monedas:
                    if not monedas.remove_balance(interaction.user.id, amount):
                        await interaction.response.send_message(
                            "❌ No tienes suficiente dinero.", ephemeral=True
                        )
                        return
                ...
                self._save_data()
                await interaction.response.send_message(
                    f"💰 Depositaste {amount} en la cuenta conjunta."
                )
            else:
                withdraw = abs(amount)
                ...
                if current < withdraw:
                    await interaction.response.send_message(
                        "❌ No hay suficientes fondos en la cuenta conjunta.", ephemeral=True
                    )
                    return
                ...
                self._save_data()
                await interaction.response.send_message(
                    f"💸 Retiraste {withdraw} de la cuenta conjunta."
                )

        elif accion == "ship":
            if not objetivo:
                await interaction.response.send_message(
                    "⚠️ Menciona a alguien para calcular compatibilidad.", ephemeral=True
                )
                return
            ...
            embed = discord.Embed(
                ...
                description=f"{interaction.user.mention} ❤️ {objetivo.mention}",
                color=Theme.get_color(interaction.guild.id, "secondary"),
            )

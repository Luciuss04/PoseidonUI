...
def generar_color_unico():
    """Genera un color aleatorio pero con tonos rituales (evita grises apagados)."""
...
class CrearRolesGuardian(commands.Cog):
    ...
    @app_commands.command(name="crear_roles_guardian", description="Crea automáticamente los roles del Guardian con colores dinámicos")
        ...
        for titulo, rol_nombre in JUICIO_ROLES.items():
            ...
            if rol:
                ...
                try:
                    ...
                    await interaction.guild.create_role(name=rol_nombre, color=color, reason="Roles del Guardian")
                    creados.append(f"{rol_nombre} ({color})")
                except discord.Forbidden:
                    await interaction.response.send_message("⚠️ No tengo permisos para crear roles.", ephemeral=True)
                    return
